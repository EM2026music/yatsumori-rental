-- ============================================================
-- 10: 予約者ごとに「お名前・ニックネーム」と「レンタルセット種別」を追加
-- 実行方法: SupabaseダッシュボードのSQL Editorに全文貼り付けて実行
-- nickname は任意（NULL可）。set_type は 'gliding'（滑走セット）/ 'full'（フルセット）。
-- 既存予約は NULL のまま残る。何度実行してもOK（冪等）。
-- ============================================================

-- 1) 列を追加
alter table public.reservation_members
  add column if not exists nickname text,     -- お名前・ニックネーム（任意）
  add column if not exists set_type text;      -- 'gliding' | 'full'（レンタルセット種別）

-- 2) 予約作成v2を更新（メンバーjsonbから nickname / set_type も保存）。署名は不変。
...
begin
  ...
  for v_m in select * from jsonb_array_elements(p_members) loop
    ...
    insert into reservation_members (reservation_id, member_no, height_cm, shoe_cm, board_type, is_child,
            age, gender, stance, nickname, set_type)
...
-- 3) 予約照会v2も nickname / set_type を返す
...
begin
  select jsonb_build_object(
    ...
    'members', coalesce((select jsonb_agg(jsonb_build_object(
        ...
        'nickname', m.nickname, 'set_type', m.set_type) order by m.member_no)
