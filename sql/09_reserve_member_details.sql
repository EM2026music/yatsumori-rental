-- ============================================================
-- 09: 予約者ごとに 年齢・性別・スタンス を追加（漫画の案内項目に合わせる）
-- 実行方法: SupabaseダッシュボードのSQL Editorに全文貼り付けて実行
-- いずれも任意項目（NULL可）。既存予約はNULLのまま残る。何度実行してもOK。
-- ============================================================

-- 1) 列を追加（任意項目なのでNULL可・チェック制約は付けずRPC/UI側で管理）
alter table public.reservation_members
  add column if not exists age    integer,   -- 年齢（開放値の参考。任意）
  add column if not exists gender text,       -- 'male' | 'female'（任意）
  add column if not exists stance text;       -- 'regular' | 'goofy' | 'unknown'（スノボのみ・任意）

-- 2) 予約作成v2を更新（メンバーjsonbから age/gender/stance も保存）
--    署名は不変なのでgrantは既存のまま有効。
...
begin
  ...
  for v_m in select * from jsonb_array_elements(p_members) loop
    v_no := v_no + 1;
    insert into reservation_members (reservation_id, member_no, height_cm, shoe_cm, board_type, is_child, age, gender, stance)
...
-- 3) 予約照会v2も age/gender/stance を返すよう更新
...
begin
  select jsonb_build_object(
    ...
    'members', coalesce((select jsonb_agg(jsonb_build_object(
        ...
        'age', m.age, 'gender', m.gender, 'stance', m.stance) order by m.member_no)
